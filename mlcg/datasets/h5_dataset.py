...
__all__ = ["MolData", "MetaSet", "Partition", "H5Dataset", "H5PartitionDataLoader"]

class MolData:
    """Data-holder for coordinates, forces and embedding vector of a molecule, e.g., opep_0000."""
    def __init__(self, name: str, embeds: np.ndarray,
                 coords: np.ndarray, forces: np.ndarray):
    ...
    def __repr__(self):
        return "MolData(name=\"" + self.name + "\", N_beads=%d, N_frames=%d)" % (self.n_beads, self.n_frames)
...
class MetaSet:
    ...
    @staticmethod
    def create_from_hdf5_group(hdf5_group, mol_list,
        ...
        def select_for_rank(length_or_indices):
            ...
            if isinstance(length_or_indices, Integral): # int or np.int*
                ...
                length = len(indices)
            rank, world_size = parallel.get("rank", 0), parallel.get("world_size", 1)
            if rank < 0 or rank >= world_size:
                raise ValueError("Rank %d is invalid given the world_size %d." % (rank, world_size))
            if length < world_size:
                raise ValueError("Length %d is too short given the world_size %d." % (length, world_size))
        ...
        for mol_name in mol_list:
            if mol_name not in hdf5_group:
                raise KeyError("`mol_list` includes moleucle %s,"
                               " which is not in the h5 group at path %s"
                               % (mol_name, hdf5_group.name))
            embeds = retrieve_hdf(hdf5_group[mol_name], keys["embeds"])
            if detailed_indices is not None and detailed_indices.get(mol_name) is not None:
            ...
            if not split_per_index:
                coords = retrieve_hdf(hdf5_group[mol_name], keys["coords"])[selection]
                forces = retrieve_hdf(hdf5_group[mol_name], keys["forces"])[selection]
            else:
    ...
    def trim_down_to(self, target_n_samples, random_seed=42):
        ...
        indices = rng.choice(self.n_total_samples, size=target_n_samples, replace=False)
        indices.sort()
        ...
        for i, next_start_index in enumerate(self._cumulate_indices):
            ...
            self._mol_dataset[i].sub_sample(indices[left_bound:right_bound] - base_index)
    ...
    def _update_info(self):
        self._n_mol_samples = np.array([mol_d.n_frames for mol_d in self._mol_dataset])
    ...
    def __repr__(self):
        return "Metaset with %d molecules and %d samples" % (self.n_mol, self.n_total_samples)
...
class Partition:
    ...
    def sampling_setup(self, batch_sizes: typing.Dict[str, MetaSet],
                       max_epoch_samples=None, random_seed=42):
        ...
        if set(batch_sizes) != set(self._metasets):
            raise ValueError("Input `batch_sizes` shall have exactly the same keys"
        ...
        max_batches = [int(sample_sizes[k] // batch_sizes[k]) for k in batch_sizes]
        ...
        for k in self._metasets:
            self._metasets[k].trim_down_to(n_batches * batch_sizes[k], random_seed=random_seed)
...
class H5Dataset:
    ...
    def __init__(self, h5_path, partition_options, loading_options):
        ...
        self._metaset_entries = {} # dict containing all metasets in the HDF5 file
    ...
    def _create_partitions(self, partition_options, loading_options):
        ...
        parallel = loading_options.get("parallel",
                                       {
                                           "rank": 0,
                                           "world_size": 1,
        ...
        for part_name in partition_options:
            ...
            for metaset_name in part_info["metasets"]:
                if metaset_name not in self._metaset_entries:
                    raise KeyError("`partition_options` refer to metaset %s,"
                                     " which is not in the h5 dataset at path %s"
                                     % (part, self._h5_path))
                mol_list = part_info["metasets"][metaset_name]["molecules"]
                detailed_indices = part_info["metasets"][metaset_name].get("detailed_indices", None)
                stride = part_info["metasets"][metaset_name].get("stride", 1)
                part.add_metaset(metaset_name,
                                 MetaSet.create_from_hdf5_group(self._metaset_entries[metaset_name],
                                                                mol_list,
                                                                detailed_indices,
                                                                stride=stride,
            ...
            if max_epoch_samples is not None and parallel["world_size"] > 1:
                max_epoch_samples = int(max_epoch_samples // parallel["world_size"])
            part.sampling_setup(part_info["batch_sizes"], max_epoch_samples, random_seed=random_seed)
...
class H5PartitionDataLoader:
    ...
    def __init__(self, data_partition, collater_fn=PyGCollater(None, None), pin_memory=False):
        self._data_part = data_partition
        self._metasets = []
        self._samplers = []
        self._pin_memory = pin_memory
        for (metaset_name, batch_size) in data_partition.batch_sizes.items():
            metaset = data_partition.get_metaset(metaset_name) # automatically checks whether the partition is sample_ready()
    ...
    def __next__(self):
        ...
        try:
            merged_samples = tuple(itertools.chain.from_iterable(
                                ((dts[i] for i in next(si)) for dts, si in zip(self._metasets, self._sampler_iters))))
