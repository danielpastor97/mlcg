...
class AlanineDataset(InMemoryDataset):
    ...
    def make_data_slices_prior(self, coords_forces_file, topology, cg_mapping, cg_topo, frac_dataset=1):
        ...
        embeddings, masses, cg_matrix, cg_mapping_ = build_cg_matrix(topology, 
        ...
        for i in range(int(len(coords)*frac_dataset)):
            ...
            ## MAKE ATOMICDATA OBJECT
            atomicData = AtomicData.from_points(atom_types=z,
                                                pos=pos,
                                                forces=force,
                                                masses=masses,
        ...
        data_list_coll, _, _ = collate( data_list[0].__class__, 
                                       data_list=data_list,
                                       increment=True,
                                       add_batch=True
                                       )
        ...
        baseline_models = self.make_baseline_models( data_list_coll, 
                                                    self.beta, 
                                                    self.priors_cls 
                                                    )
        ...
        data_list_coll, slices, _ = collate( data_list[0].__class__, 
                                            data_list=data_list,
                                            increment=False,
                                            add_batch=False
                                            )
    ...
    def make_cg_topology(topology, cg_mapping=AL_CG_MAP, special_terminal=False):
        ...
        cg_topo = build_cg_topology(topology, 
                                    cg_mapping=cg_mapping, 
    ...
    def make_baseline_models( data, beta, priors_cls ):
        ...
        """
        baseline_models, statistics = fit_baseline_models( data, 
                                                          beta, 
                                                          priors_cls
                                                          )
        ...
        for k in baseline_models.keys():
            baseline_models[k] = GradientsOut(baseline_models[k], targets="forces")
    ...
    def process(self, cg_mapping=AL_CG_MAP ):
        ...
        ## MAKE CG TOPOLOGY
        topologies = self.make_cg_topology(topology, 
                                                cg_mapping=cg_mapping,
                                                special_terminal=False
                                                )
        
        ## MAKE ATOMIC DATA OBJECT AND ATOM SLICES
        data, slices, prior_models = self.make_data_slices_prior( self.raw_paths[0], 
                                                                 topology, 
                                                                 cg_mapping,
                                                                 topologies,
