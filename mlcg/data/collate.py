from typing import List, NamedTuple, Tuple, Optional, Union, Any
...
def _collate(
    ...
    elif isnamedtupleinstance(elem):
        value_dict, slice_dict, inc_dict = {}, {}, {}
        for key in elem._fields:
            value_dict[key], slice_dict[key], inc_dict[key] = _collate(
            key,[ v.__getattribute__(key) for v in values], data_list, stores, increment)
        cls = type(elem)
        value = cls(**value_dict)
        return value, slice_dict, inc_dict
...
def isnamedtupleinstance(x):
    ...
    if len(b) != 1 or b[0] != tuple:
        return False
    if getattr(t, '__getattribute__', None) is None:
        return False
    f = getattr(t, '_fields', None)
