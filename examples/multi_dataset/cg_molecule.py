...
# https://github.com/coarse-graining/cgnet/blob/master/cgnet/molecule/trajectory.py
...
class CGMolecule:
    ...
    def __init__(
        ...
        if not (len(names) == len(resseq) == len(chainseq)):
            raise ValueError(
                "Names and resseq must be lists of the same length"
            )
        self.names = names
        self.resseq = resseq
        self.chainseq = chainseq
